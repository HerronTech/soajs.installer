...
function cb200(cb) {
  return function (err, result) {
    ...
    if (result.statusCode < 200 || result.statusCode > 299) {
      const error = new Error((result.body && result.body.message) ? result.body.message : (result.body) ? result.body : result);
      error.code = ( result.body && result.body.code) ? result.body.code : result.statusCode;
      return cb(error);
    }
    cb(null, result.body);
  };
